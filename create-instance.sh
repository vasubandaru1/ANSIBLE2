...
aws ec2 run-instances --launch-template LaunchTemplateId=lt-0c2f23ab38ce47240,Version=03 --tag-specifications "ResourceType=instance,Tags=[{Key=Name,Value=$1}]" "ResourceType=spot-instances-request,Tags=[{Key=Name,Value=$1}]" | jq &>/dev/null
